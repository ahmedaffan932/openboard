...
public final class InputLogic {
     ...
     *                                    dictionary.
     */
    public InputLogic(final LatinIME latinIME, final SuggestionStripViewAccessor suggestionStripViewAccessor, final DictionaryFacilitator dictionaryFacilitator) {
        mLatinIME = latinIME;
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        if (settingsValues.mShouldShowLxxSuggestionUi) {
            mConnection.requestCursorUpdates(true /* enableMonitor */, true /* requestImmediateCallback */);
        }
    }
...
//    }
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues, @Nonnull final Event event, final int keyboardShiftMode, final int currentKeyboardScriptId, final LatinIME.UIHandler handler) {
        ...
        if (!inputTransaction.didAutoCorrect() && processedEvent.getMKeyCode() != Constants.CODE_SHIFT && processedEvent.getMKeyCode() != Constants.CODE_CAPSLOCK && processedEvent.getMKeyCode() != Constants.CODE_SWITCH_ALPHA_SYMBOL)
            mLastComposedWord.deactivate();
    ...
    public void onStartBatchInput(final SettingsValues settingsValues, final KeyboardSwitcher keyboardSwitcher, final LatinIME.UIHandler handler) {
        ...
        handler.showGesturePreviewAndSuggestionStrip(SuggestedWords.getEmptyInstance(), false /* dismissGestureFloatingPreviewText */);
        ...
        mConnection.endBatchEdit();
        mWordComposer.setCapitalizedModeAtStartComposingTime(getActualCapsMode(settingsValues, keyboardSwitcher.getKeyboardShiftMode()));
    }
    ...
    public void onCancelBatchInput(final LatinIME.UIHandler handler) {
        ...
        handler.showGesturePreviewAndSuggestionStrip(SuggestedWords.getEmptyInstance(), true /* dismissGestureFloatingPreviewText */);
    }
        ...
        switch (event.getMKeyCode()) {
            ...
            case Constants.CODE_CLIPBOARD:
                ...
                if (!inputTransaction.getMSettingsValues().mClipboardHistoryEnabled) {
                    final CharSequence content = mLatinIME.getClipboardHistoryManager().retrieveClipboardContent();
            ...
            case Constants.CODE_SHIFT_ENTER:
                final Event tmpEvent = Event.createSoftwareKeypressEvent(Constants.CODE_ENTER, event.getMKeyCode(), event.getMX(), event.getMY(), event.isKeyRepeat());
    ...
    private void handleNonSpecialCharacterEvent(final Event event, final InputTransaction inputTransaction, final LatinIME.UIHandler handler) {
        ...
        mSpaceState = SpaceState.NONE;
        if (inputTransaction.getMSettingsValues().isWordSeparator(codePoint) || Character.getType(codePoint) == Character.OTHER_SYMBOL) {
    ...
    private void handleNonSeparatorEvent(final Event event, final SettingsValues settingsValues, final InputTransaction inputTransaction) {
        ...
        if (!isComposingWord
                ...
                (!settingsValues.mSpacingAndPunctuations.mCurrentLanguageHasSpaces || !mConnection.isCursorTouchingWord(settingsValues.mSpacingAndPunctuations, !mConnection.hasSlowInputConnection() /* checkTextAfter */))) {
    ...
    private void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction, final LatinIME.UIHandler handler) {
        ...
        if (tryPerformDoubleSpacePeriod(event, inputTransaction)) {
            ...
            if ((SpaceState.PHANTOM == inputTransaction.getMSpaceState() && settingsValues.isUsuallyFollowedBySpace(codePoint)) || (Constants.CODE_DOUBLE_QUOTE == codePoint && isInsideDoubleQuoteOrAfterDigit)) {
                ...
                if (wasComposingWord && settingsValues.mAutospaceAfterPunctuationEnabled && settingsValues.isUsuallyFollowedBySpace(codePoint)) {
                    mSpaceState = SpaceState.PHANTOM;
                }
     ...
     * @param inputTransaction The transaction in progress.
     */
    private void handleBackspaceEvent(final Event event, final InputTransaction inputTransaction, final int currentKeyboardScriptId) {
        mSpaceState = SpaceState.NONE;
        mDeleteCount++;
        ...
        if (mWordComposer.isComposingWord()) {
            if (mWordComposer.isBatchMode()) {
                ...
                if (!TextUtils.isEmpty(rejectedSuggestion)) {
                    unlearnWord(rejectedSuggestion, inputTransaction.getMSettingsValues(), Constants.EVENT_REJECTION);
                }
            ...
            if (mLastComposedWord.canRevertCommit()) {
                ...
                if (inputTransaction.getMSettingsValues().isSuggestionsEnabledPerUserSettings() && inputTransaction.getMSettingsValues().mSpacingAndPunctuations.mCurrentLanguageHasSpaces && !mConnection.isCursorFollowedByWordCharacter(inputTransaction.getMSettingsValues().mSpacingAndPunctuations)) {
                    restartSuggestionsOnWordTouchedByCursor(inputTransaction.getMSettingsValues(), false /* forStartInput */, currentKeyboardScriptId);
                }
                return;
            }
            ...
            if (mConnection.hasSelection()) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    unlearnWord(selection.toString(), inputTransaction.getMSettingsValues(), Constants.EVENT_BACKSPACE);
                ...
                if (inputTransaction.getMSettingsValues().mInputAttributes.isTypeNull() || Constants.NOT_A_CURSOR_POSITION == mConnection.getExpectedSelectionEnd()) {
                    ...
                    if (mDeleteCount > Constants.DELETE_ACCELERATE_AT) {
                        ...
                        hasUnlearnedWordBeingDeleted |= unlearnWordBeingDeleted(inputTransaction.getMSettingsValues(), currentKeyboardScriptId);
                        sendDownUpKeyEvent(KeyEvent.KEYCODE_DEL);
                        totalDeletedLength++;
                    }
            ...
            if (!hasUnlearnedWordBeingDeleted) {
                ...
                unlearnWordBeingDeleted(inputTransaction.getMSettingsValues(), currentKeyboardScriptId);
            }
            if (mConnection.hasSlowInputConnection()) {
                mSuggestionStripViewAccessor.setNeutralSuggestionStrip();
            } else if (inputTransaction.getMSettingsValues().isSuggestionsEnabledPerUserSettings() && inputTransaction.getMSettingsValues().mSpacingAndPunctuations.mCurrentLanguageHasSpaces && !mConnection.isCursorFollowedByWordCharacter(inputTransaction.getMSettingsValues().mSpacingAndPunctuations)) {
                restartSuggestionsOnWordTouchedByCursor(inputTransaction.getMSettingsValues(), false /* forStartInput */, currentKeyboardScriptId);
            }
        }
    }
    ...
    boolean unlearnWordBeingDeleted(final SettingsValues settingsValues, final int currentKeyboardScriptId) {
        if (mConnection.hasSlowInputConnection()) {
    ...
    private boolean tryStripSpaceAndReturnWhetherShouldSwapInstead(final Event event, final InputTransaction inputTransaction) {
        ...
        if (Constants.CODE_ENTER == codePoint && SpaceState.SWAP_PUNCTUATION == inputTransaction.getMSpaceState()) {
            mConnection.removeTrailingSpace();
            return false;
        }
        if ((SpaceState.WEAK == inputTransaction.getMSpaceState() || SpaceState.SWAP_PUNCTUATION == inputTransaction.getMSpaceState()) && isFromSuggestionStrip) {
    ...
    public boolean isDoubleSpacePeriodCountdownActive(final InputTransaction inputTransaction) {
        return inputTransaction.getMTimestamp() - mDoubleSpacePeriodCountdownStart < inputTransaction.getMSettingsValues().mDoubleSpacePeriodTimeout;
    }
    ...
    private void performRecapitalization(final SettingsValues settingsValues) {
        ...
        if (!mRecapitalizeStatus.isStarted() || !mRecapitalizeStatus.isSetAt(selectionStart, selectionEnd)) {
            ...
            mRecapitalizeStatus.start(selectionStart, selectionEnd, selectedText.toString(), settingsValues.mLocale, settingsValues.mSpacingAndPunctuations.mSortedWordSeparators);
    ...
    private void performAdditionToUserHistoryDictionary(final SettingsValues settingsValues, final String suggestion, @Nonnull final NgramContext ngramContext) {
        ...
        if (!settingsValues.mAutoCorrectionEnabledPerUserSettings || settingsValues.mIncognitoModeEnabled)
            return;
        if (mConnection.hasSlowInputConnection()) {
    ...
    public void performUpdateSuggestionStripSync(final SettingsValues settingsValues, final int inputStyle) {
        ...
        mInputLogicHandler.getSuggestedWords(inputStyle, SuggestedWords.NOT_A_SEQUENCE_NUMBER, new OnGetSuggestedWordsCallback() {
            ...
            public void onGetSuggestedWords(final SuggestedWords suggestedWords) {
                ...
                final SuggestedWordInfo typedWordInfo = new SuggestedWordInfo(typedWordString, "" /* prevWordsContext */, SuggestedWordInfo.MAX_SCORE, SuggestedWordInfo.KIND_TYPED, Dictionary.DICTIONARY_USER_TYPED, SuggestedWordInfo.NOT_AN_INDEX /* indexOfTouchPointOfSecondWord */, SuggestedWordInfo.NOT_A_CONFIDENCE);
     ...
     */
    public void restartSuggestionsOnWordTouchedByCursor(final SettingsValues settingsValues, final boolean forStartInput,
        ...
        final SuggestedWordInfo typedWordInfo = new SuggestedWordInfo(typedWordString, "" /* prevWordsContext */, SuggestedWords.MAX_SUGGESTIONS + 1, SuggestedWordInfo.KIND_TYPED, Dictionary.DICTIONARY_USER_TYPED, SuggestedWordInfo.NOT_AN_INDEX /* indexOfTouchPointOfSecondWord */, SuggestedWordInfo.NOT_A_CONFIDENCE /* autoCommitFirstWordConfidence */);
        ...
        for (final SuggestionSpan span : range.getSuggestionSpansAtWord()) {
            for (final String s : span.getSuggestions()) {
                ...
                if (!TextUtils.equals(s, typedWordString)) {
                    suggestions.add(new SuggestedWordInfo(s, "" /* prevWordsContext */, SuggestedWords.MAX_SUGGESTIONS - i, SuggestedWordInfo.KIND_RESUMED, Dictionary.DICTIONARY_RESUMED, SuggestedWordInfo.NOT_AN_INDEX /* indexOfTouchPointOfSecondWord */, SuggestedWordInfo.NOT_A_CONFIDENCE
        ...
        if (suggestions.size() <= 1) {
            ...
            final SuggestedWords suggestedWords = new SuggestedWords(suggestions, null /* rawSuggestions */, typedWordInfo, false /* typedWordValid */, false /* willAutoCorrect */, false /* isObsoleteSuggestions */, SuggestedWords.INPUT_STYLE_RECORRECTION, SuggestedWords.NOT_A_SEQUENCE_NUMBER);
            doShowSuggestionsAndClearAutoCorrectionIndicator(suggestedWords);
        }
    }
     ...
     * @param settingsValues   the current values of the settings.
     */
    private void revertCommit(final InputTransaction inputTransaction, final SettingsValues settingsValues) {
        final CharSequence originallyTypedWord = mLastComposedWord.mTypedWord;
        final String originallyTypedWordString = originallyTypedWord != null ? originallyTypedWord.toString() : "";
        ...
        if (committedWord instanceof SpannableString) {
            ...
            for (final Object span : spans) {
                ...
                if (span instanceof SuggestionSpan) {
                    ...
                    textToCommit.setSpan(span, 0 /* start */, lastCharIndex /* end */, committedWordWithSuggestionSpans.getSpanFlags(span));
                }
            }
            // Add the suggestion list to the list of suggestions.
            textToCommit.setSpan(new SuggestionSpan(mLatinIME /* context */, inputTransaction.getMSettingsValues().mLocale, suggestions.toArray(new String[suggestions.size()]), 0 /* flags */, null /* notificationTargetClass */), 0 /* start */, lastCharIndex /* end */, 0 /* flags */);
        }
     ...
     */
    private int getActualCapsMode(final SettingsValues settingsValues, final int keyboardShiftMode) {
    ...
    private CharSequence getTextWithUnderline(final String text) {
        ...
        return mIsAutoCorrectionIndicatorOn ? SuggestionSpanUtils.getTextWithAutoCorrectionIndicatorUnderline(mLatinIME, text, getDictionaryFacilitatorLocale()) : text;
    }
    ...
    private void insertAutomaticSpaceIfOptionsAndTextAllow(final SettingsValues settingsValues) {
        if (settingsValues.shouldInsertSpacesAutomatically() && settingsValues.mSpacingAndPunctuations.mCurrentLanguageHasSpaces && !mConnection.textBeforeCursorLooksLikeURL()) {
    ...
    public void getSuggestedWords(final SettingsValues settingsValues, final Keyboard keyboard, final int keyboardShiftMode, final int inputStyle, final int sequenceNumber, final OnGetSuggestedWordsCallback callback) {
        mWordComposer.adviseCapitalizedModeBeforeFetchingSuggestions(getActualCapsMode(settingsValues, keyboardShiftMode));
        mSuggest.getSuggestedWords(mWordComposer, getNgramContextFromNthPreviousWordForSuggestion(settingsValues.mSpacingAndPunctuations,
                // Get the word on which we should search the bigrams. If we are composing
                // a word, it's whatever is *before* the half-committed word in the buffer,
                // hence 2; if we aren't, we should just skip whitespace if any, so 1.
                mWordComposer.isComposingWord() ? 2 : 1), keyboard, new SettingsValuesForSuggestion(settingsValues.mBlockPotentiallyOffensive), settingsValues.mAutoCorrectionEnabledPerUserSettings, inputStyle, sequenceNumber, callback);
    }
